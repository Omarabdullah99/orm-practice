        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form class="max-w-sm mx-auto" method="POST" action="{{ route('products.update', $product) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="description" name="description"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Product Description" value="{{ old('description', $product->description) }}" />
                            @error('description')
                                <div class="text-red-400 my-3">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @selected(old('user_id', $product->user->id) == $user->id)>
                                        {{ $user->name }}</option>
                                @endforeach
                            </select>
